...
function Level2Game() {
  ...
  const [timeLeft, setTimeLeft] = useState(60); // 60 seconds for level 2
  ...
  const [level, setLevel] = useState(2); // Fixed at level 2
  ...
  useEffect(() => {
    ...
    let currentIndex = 0;
    // Spawn obstacles at a slightly faster rate than level 1 (2.8 seconds instead of 3)
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 2800); // Slightly faster spawn rate for level 2
    ...
    const moveObstacles = setInterval(() => {
      setObstacleList((prevObstacles) => {
        const updatedObstacles = prevObstacles.map((obstacle) => ({
          ...
          position: {
            ...obstacle.position,
            left: `${parseFloat(obstacle.position.left) - 1.2}%`, // Slightly faster movement
          },
        }));
  ...
  const levelPassed = successRate >= 100; // 70% success rate required to pass
      ...
      <div className="game-container" style={{
        ...
        {gameOver && (
          <div style={{
            ...
            {levelPassed ? (
              <p style={{ color: "#4CAF50" }}>Level Passed! You can proceed to Level 3!</p>
            ) : (
