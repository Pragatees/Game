...
const level6Words = [
  // 4 letter words
  "grid", "fuel", "swim", "task", "axis", "diet", "gaze", "path", "risk", "zone",
  // 5 letter words
  "climb", "focus", "trust", "blend", "drive", "float", "smart", "voice", "quest", "yield"
];
...
function Level6Game() {
  ...
  const [obstacleSpeed, setObstacleSpeed] = useState(1.6); // Slightly higher base speed for Level 6
  ...
  useEffect(() => {
    if (timeLeft > 0 && !gameOver && !allWordsCompleted) {
      const timer = setInterval(() => {
        ...
        setObstacleSpeed(prev => Math.min(prev + 0.02, 2.7)); // More aggressive speed increase
      }, 1000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 1800); // Even faster spawn rate for level 6
  ...
  const handleWordMatch = () => {
    ...
    // Enhanced combo system for Level 6 - faster combo buildup
    if (newConsecutiveCorrect % 2 === 0) {
      setComboMultiplier(prev => Math.min(prev + 1.2, 6)); // Higher max multiplier
    }
    ...
    <div className="App" style={{ backgroundColor: "black", height: "100vh" }}>
      <div className="game-container" style={{
        ...
        <div style={{
          ...
          top: "120px",
          right: "20px",
          color: obstacleSpeed > 2.2 ? "#ff4444" : "#3F51B5",
          fontSize: "20px",
          fontWeight: "bold",
          background: "rgba(0,0,0,0.7)",
          padding: "10px",
        ...
        {gameOver && (
          <div style={{
            ...
            {levelPassed ? (
              <p style={{ color: "#3F51B5" }}>Level Passed! You can proceed to Level 7!</p>
            ) : (
            ...
            {levelPassed && (
              <button
                onClick={() => window.location.href = "/level7"}
                style={{
                  ...buttonBaseStyle,
